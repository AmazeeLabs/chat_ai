...
class SettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    // Debug
    // $path = '/Users/dimitris.spachos/Sites/paraplegie_ch/apps/cms/web/sites/default/files/chat_ai_files_custom/Paraplegie_04_2024_de.pdf';

    // $supabase = \Drupal::service('chat_ai.supabase');
    // $chunks = $supabase->getMultiQueryMatchingChunks('Should I offer help to people on wheelchairs?');

    /*
    $file = \Drupal\file\Entity\File::load(60865);
    if ($file) {
    ...
    $question = 'Auch Ageing wird bedeutender?';
    $response = $chat->getMultiQuery($question);
    $text = $response[0] ?: NULL;
    if ($text) {
      $supabase = \Drupal::service('chat_ai.supabase');
      $chunks = $supabase->getMultiQueryMatchingChunks($question);
      dpm($chunks);
      $array = explode("\n", $text);
      $array = array_map('trim', $array);
      $array = array_filter($array);
      foreach ($array as $item) {
        dpm($supabase->getMatchingChunks($item));
      }
    }
    */
    //dpm($supabase->getMatchingChunks('Was ist der Cybathlon 2024?'));
    // dpm($supabase->getMatchingChunks('Auch Ageing wird bedeutender?'));
    // dpm($supabase->getMatchingChunks('Was fasziniert Sie persönlich an der Arbeit im SPZ?'));

    // $entity = \Drupal\node\Entity\Node::load(849);
    // $embeddings->deleteQueueItem($entity);
    //...
